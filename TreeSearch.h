...
template <class T>
unsigned int bitCount (T pat)
{
  return pat.count();
}

template <>
inline unsigned int bitCount(unsigned long long pat)
{
  unsigned int c=0;
  while (pat)
    {
...
class TreeSearch
{
 ...
 private:
  ...
  void recurseEnumerateMatch(int lv, entry *e, unsigned int N) // will be tolerant of up to N missing cells
  {
    while(true) 
      {
	if ( bitCount<T>(( ptm[lv] ^ e->pattern)&e->pattern) < (N+1))
	  {
	    if (lv==0) 
	      // found a match, save it to library.
	      matches.push_back(&(e->pattern));
	    else
	      recurseEnumerateMatch(lv-1,e+1,N);
	  }
	if (e->skipto==0) return;
	e=&entries[e->skipto];
      }
 ...
 public:
  ...
  void enumerateMatches(T& pat, unsigned int N)
  {
    if (N==0)
      enumerateMatches(pat);
    else
      {
	matches.clear();
	preparePtm(pat);
	recurseEnumerateMatch(levels-1,entries,N);
      }
  }
